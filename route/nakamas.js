...
routeNakamas.get("/api/v1/nakamas/name=:nakamaName", (req, res) => {
  ...
  function preparaAllData() {
    searchNakama(nakamaName.replaceAll(" ", "_")).then((content) => {
      const $ = load(content);
      ...
      res.status(200);
      res.send(JSON.stringify(prepareObj));
    });
  }
